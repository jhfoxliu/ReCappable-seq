...
def process_line(row):
    ...
    if all_exons is None:
        ...
        INFO = "{}|{}|{}|EXON|{}".format(GENE, biotype, biotype2, dist)
...
if __name__ == "__main__":
    ...
    group_required.add_argument("--thresh-min", dest="thresh_min",required=False, type=int, default=-100,help="Distance limit, downstream")
    ...
    with open(options.input2, "r") as input:
        for line in input.readlines():
            line = line.strip().split()
            chr, pos_0, pos_1, _, count, strand, _, tx_start, tx_end, enst, gene, _, bedtools_dist = line
            key = (chr, pos_0, pos_1, strand)
