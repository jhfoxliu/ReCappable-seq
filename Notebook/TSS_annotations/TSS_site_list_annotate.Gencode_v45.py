...
if __name__ == "__main__":
    ...
    group_required.add_argument("-f","--fasta",dest="fasta",required=True, default="/home/fox/Database/Genome/Human/Gencode/v45/GRCh38.primary_assembly.genome.nochr.fa")
    group_required.add_argument("-t","--tss",dest="tss_db",required=True, default="/home/fox/Database/Genome/Human/Gencode/v45/gencode.v45.primary_assembly.annotation.nochr.tss.bed",help="")
    group_required.add_argument("-e","--exons",dest="exons_db",required=True, default="/home/fox/Database/Genome/Human/Gencode/v45/gencode.v45.primary_assembly.annotation.nochr.tx.bed",help="")
    group_required.add_argument("-a","--annot",dest="annot",required=True, default="/home/fox/Database/Genome/Human/Gencode/v45/gencode.v45.primary_assembly.annotation.nochr.anno",help="")
    ...
    group_required.add_argument("--bedtools",dest="bedtools",required=False, default="bedtools",help="")
    group_required.add_argument("--blastn",dest="blastn",required=False, default="blastn",help="")
    ...
    cmd = "python genome_flanking_v3.py -U 0 -D 50 -f {fasta} -i {sites_in} > {fasta_out}".format(
        fasta = options.fasta,
    ...
    # with open(options.output + ".unsorted.bed", "w") as bed_out:
    #     for idx, row in df.iterrows():
    #         bed_out.write("{}\t{}\t{}\t{}\t{}\t{}\n".format(idx[0], idx[1] - 1, idx[1], ".", ".", idx[2]))

    # # sort bedfile
    # cmd = "{bedtools} sort -i {bed_in} > {bed_out}".format(bed_in = options.output + ".unsorted.bed", bed_out = options.output + ".bed", bedtools=bedtools)
    # print(cmd)
    # os.system(cmd)
    ...
    cmd = "python collaspe_bed_annotations_v3.py -i {closest} -a {annot} -o {collasped}".format(closest= options.output + ".closest_tss.bed", annot=options.annot, collasped = options.output + ".collasped.1.bed")
    print(cmd)
    os.system(cmd)

    cmd = "python collaspe_bed_annotations_fix_other_exons_v2.py -a {annot} -i {collasped} -I {closest_exons} -o {collasped_2} ".format(collasped = options.output + ".collasped.1.bed", closest_exons=options.output + ".closest_exons.bed", annot=options.annot, collasped_2= options.output + ".collasped.2.bed")
    print(cmd)
    os.system(cmd)

    cmd = "python collaspe_bed_annotations_fix_upstream_TSS.py  --thresh-min -2000 --thresh-max 2000 -a {annot} -i {collasped} -I {closest_exons} -o {collasped_final} ".format(collasped = options.output + ".collasped.2.bed", closest_exons=options.output + ".closest_tss.upstream.bed", annot=options.annot, collasped_final= options.output + ".collasped_final.bed")
    print(cmd)
    os.system(cmd)
